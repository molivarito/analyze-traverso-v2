...
from PyQt5.QtWidgets import (
    ...
    QHeaderView, QFormLayout, QSpinBox
)
...
class FluteGeometryEditor(QDialog):
    ...
    def __init__(self, flute_data: Any, parent=None):
        ...
        self.acoustic_analysis_calculated = False
    ...
    def _create_left_panel(self) -> QWidget:
        ...
        # Parámetros acústicos
        acoustic_group = QGroupBox("Parámetros Acústicos")
        acoustic_layout = QFormLayout(acoustic_group)
        
        self.temp_spinbox = QSpinBox()
        self.temp_spinbox.setRange(-10, 40)
        self.temp_spinbox.setValue(int(self.temperature))
        self.temp_spinbox.setSuffix(" °C")
        acoustic_layout.addRow("Temperatura:", self.temp_spinbox)
        
        self.la_freq_spinbox = QSpinBox()
        self.la_freq_spinbox.setRange(392, 466)
        self.la_freq_spinbox.setValue(int(self.la_frequency))
        self.la_freq_spinbox.setSuffix(" Hz")
        acoustic_layout.addRow("La (diapasón):", self.la_freq_spinbox)
        ...
        recalc_btn = QPushButton("🔄 Recalcular Análisis Acústico")
        recalc_btn.clicked.connect(self._recalculate_acoustic)
        actions_layout.addWidget(recalc_btn)
        
        compare_btn = QPushButton("📊 Actualizar Comparación")
        compare_btn.clicked.connect(self._update_comparison)
    ...
    def _create_center_panel(self) -> QWidget:
        ...
        self.edit_figure = Figure(figsize=(8, 4), dpi=100)
        ...
        # Gráfico de comparación acústica
        comparison_group = QGroupBox("Comparación Acústica: Original vs Modificado")
        ...
        comp_toolbar = NavigationToolbar(self.comparison_canvas, self)
        comparison_layout.addWidget(comp_toolbar)
    ...
    def _create_right_panel(self) -> QWidget:
        ...
        self.holes_table.setHorizontalHeaderLabels(["Posición", "Diámetro", "Chimenea", "Diam. Ext."])
        ...
        comparison_group = QGroupBox("Comparación Numérica")
        comparison_layout = QVBoxLayout(comparison_group)
        
        self.comparison_table = QTableWidget()
        self.comparison_table.setColumnCount(4)
        self.comparison_table.setHorizontalHeaderLabels(["Nota", "Frec. Orig (Hz)", "Frec. Mod (Hz)", "Diff (cents)"])
    ...
    def _update_holes_table(self):
        ...
        for i in range(num_holes):
            ...
            diam_item = QTableWidgetItem(f"{holes_diam[i]:.3f}" if i < len(holes_diam) else "0.0")
            ...
            self.holes_table.setItem(i, 2, chim_item)
            self.holes_table.setItem(i, 3, diam_out_item)
    ...
    def _update_edit_plot(self):
        ...
        if holes_pos:
            ...
            for i, (h_pos, h_diam) in enumerate(zip(holes_pos, holes_diam)):
                # Interpolar diámetro del perfil en la posición del agujero
                profile_diam = np.interp(h_pos, positions, diameters)
                self.edit_ax.plot(h_pos, profile_diam, 'go', markersize=12, picker=10)
    ...
    def _on_holes_table_changed(self, row, column):
        ...
        try:
            ...
            if column == 0:
                holes_pos[row] = float(self.holes_table.item(row, 0).text())
            elif column == 1:
                holes_diam[row] = float(self.holes_table.item(row, 1).text())
            elif column == 2:
                holes_chim[row] = float(self.holes_table.item(row, 2).text())
            elif column == 3:
                holes_diam_out[row] = float(self.holes_table.item(row, 3).text())
    ...
    def _recalculate_acoustic(self):
        """Recalcula el análisis acústico para la geometría modificada."""
        self._set_status("Calculando análisis acústico...")
        QApplication.processEvents()
        
        try:
            # Crear una instancia temporal de FluteData con los datos modificados
            ...
            self.modified_flute_data = FluteData(
                ...
                source_name=f"{self.flute_name}_modified",
                temperature=self.temperature,
                la_frequency=self.la_frequency
            )
            
            self.acoustic_analysis_calculated = True
            self._set_status("Análisis acústico calculado")
            self._update_comparison()
        
        except Exception as e:
            logger.error(f"Error calculando análisis acústico: {e}", exc_info=True)
            QMessageBox.critical(self, "Error", f"Error calculando análisis acústico:\n{e}")
            self._set_status("Error en cálculo")
    
    def _update_comparison(self):
        """Actualiza los gráficos y tablas de comparación acústica."""
        if not self.acoustic_analysis_calculated or not self.modified_flute_data:
            QMessageBox.information(
                self, "Análisis no disponible",
                "Primero debe recalcular el análisis acústico para la geometría modificada."
            )
            return
    ...
    def _update_comparison_plots(self):
        """Actualiza los gráficos de comparación acústica."""
        ...
        ax3 = self.comparison_figure.add_subplot(gs[1, :])
        
        # Seleccionar una nota para comparar (por ejemplo, la primera disponible)
        orig_analysis = self.original_flute_data.acoustic_analysis
        mod_analysis = self.modified_flute_data.acoustic_analysis
        
        if not orig_analysis or not mod_analysis:
            ax1.text(0.5, 0.5, "No hay análisis disponible", 
                    ha='center', va='center', transform=ax1.transAxes)
            self.comparison_canvas.draw_idle()
            return
        
        # Tomar la primera nota común
        common_notes = set(orig_analysis.keys()) & set(mod_analysis.keys())
        if not common_notes:
            ax1.text(0.5, 0.5, "No hay notas comunes", 
                    ha='center', va='center', transform=ax1.transAxes)
            self.comparison_canvas.draw_idle()
            return
        
        note = sorted(common_notes)[0]
        
        # Plot admitancia original
        try:
            freq = orig_analysis[note].frequencies
            impedance = orig_analysis[note].impedance
            admittance = np.abs(1.0 / (impedance + 1e-10))  # Evitar división por cero
            ax1.plot(freq, admittance, 'b-', linewidth=2)
            ax1.set_title(f"Original - {note}", fontsize=10)
            ax1.set_xlabel("Frecuencia (Hz)", fontsize=8)
            ax1.set_ylabel("Admitancia", fontsize=8)
            ax1.grid(True, alpha=0.3)
        except Exception as e:
            logger.warning(f"Error ploteando admitancia original: {e}")
            ax1.text(0.5, 0.5, f"Error: {e}", ha='center', va='center', transform=ax1.transAxes, color='red', fontsize=8)
        
        # Plot admitancia modificado
        try:
            freq = mod_analysis[note].frequencies
            impedance = mod_analysis[note].impedance
            admittance = np.abs(1.0 / (impedance + 1e-10))  # Evitar división por cero
            ax2.plot(freq, admittance, 'r-', linewidth=2)
            ax2.set_title(f"Modificado - {note}", fontsize=10)
            ax2.set_xlabel("Frecuencia (Hz)", fontsize=8)
            ax2.set_ylabel("Admitancia", fontsize=8)
            ax2.grid(True, alpha=0.3)
        except Exception as e:
            logger.warning(f"Error ploteando admitancia modificada: {e}")
            ax2.text(0.5, 0.5, f"Error: {e}", ha='center', va='center', transform=ax2.transAxes, color='red', fontsize=8)
        
        # Plot diferencia en inharmonicidad para todas las notas
        notes_ordered = sorted(common_notes)
        orig_cents = []
        mod_cents = []
        
        for n in notes_ordered:
            try:
                orig_antires = list(orig_analysis[n].antiresonance_frequencies())
                mod_antires = list(mod_analysis[n].antiresonance_frequencies())
                
                if len(orig_antires) >= 2:
                    f1, f2 = orig_antires[0], orig_antires[1]
                    orig_cents.append(1200 * np.log2(f2 / (2.0 * f1)) if f1 > 0 and f2 > 0 else np.nan)
                else:
                    orig_cents.append(np.nan)
                
                if len(mod_antires) >= 2:
                    f1, f2 = mod_antires[0], mod_antires[1]
                    mod_cents.append(1200 * np.log2(f2 / (2.0 * f1)) if f1 > 0 and f2 > 0 else np.nan)
                else:
                    mod_cents.append(np.nan)
            
            except Exception as e:
                logger.warning(f"Error calculando inharmonicidad para nota {n}: {e}")
                orig_cents.append(np.nan)
                mod_cents.append(np.nan)
        
        x_pos = np.arange(len(notes_ordered))
        width = 0.35
        
        ax3.bar(x_pos - width/2, orig_cents, width, label='Original', color='blue', alpha=0.7)
        ax3.bar(x_pos + width/2, mod_cents, width, label='Modificado', color='red', alpha=0.7)
        ax3.set_xticks(x_pos)
        ax3.set_xticklabels(notes_ordered, rotation=45, ha='right')
        ax3.set_ylabel("Inharmonicidad (cents)", fontsize=8)
        ax3.set_title("Comparación de Inharmonicidad", fontsize=10)
    ...
    def _update_comparison_table(self):
        """Actualiza la tabla de comparación numérica."""
        ...
        common_notes = sorted(set(orig_analysis.keys()) & set(mod_analysis.keys()))
        self.comparison_table.setRowCount(len(common_notes))
        
        for i, note in enumerate(common_notes):
            try:
                orig_antires = list(orig_analysis[note].antiresonance_frequencies())
                mod_antires = list(mod_analysis[note].antiresonance_frequencies())
                
                orig_f0 = orig_antires[0] if len(orig_antires) > 0 else 0.0
                mod_f0 = mod_antires[0] if len(mod_antires) > 0 else 0.0
                
                diff_hz = mod_f0 - orig_f0
                diff_cents = 1200 * np.log2(mod_f0 / orig_f0) if orig_f0 > 0 and mod_f0 > 0 else 0.0
                
                self.comparison_table.setItem(i, 0, QTableWidgetItem(note))
                self.comparison_table.setItem(i, 1, QTableWidgetItem(f"{orig_f0:.2f}"))
                self.comparison_table.setItem(i, 2, QTableWidgetItem(f"{mod_f0:.2f}"))
                self.comparison_table.setItem(i, 3, QTableWidgetItem(f"{diff_cents:.2f}"))
            
            except Exception as e:
                logger.warning(f"Error procesando nota {note}: {e}")
