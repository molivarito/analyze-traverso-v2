...
class FluteOperations:
                        ...
                        if h_pos_rel is not None and h_diam is not None:
                            ...
                            if abs_hole_pos not in seen_hole_positions:
                                ...
                                if isinstance(h_diam, (list, tuple)) and len(h_diam) == 2:
                                    # Cono: usar diámetro externo para el radio del círculo
                                    diam_for_radius = float(h_diam[0])
                                else:
                                    # Cilindro: usar el número directamente
                                    diam_for_radius = float(h_diam) if isinstance(h_diam, (int, float)) else 0.0
                                
                                hole_radius = diam_for_radius / 2.0
    ...
    def plot_axial_cut_2d(self, ax: Optional[plt.Axes] = None,
        ...
        # Dibujar perfil interno (superior e inferior) - zorder alto para que esté encima de los agujeros
        ax.plot(adjusted_int_positions, int_radii, 
                color=int_color, linestyle='--', linewidth=1.5, 
                label=f"{label_to_use} (interno)", zorder=15)
        ax.plot(adjusted_int_positions, -int_radii, 
                color=int_color, linestyle='--', linewidth=1.5, 
                zorder=15)
        
        # Dibujar perfil externo (superior e inferior) - zorder alto para que esté encima de los agujeros
        ax.plot(adjusted_ext_positions, ext_radii, 
                color=ext_color, linestyle='-', linewidth=1.5, 
                label=f"{label_to_use} (externo)", zorder=15)
        ax.plot(adjusted_ext_positions, -ext_radii, 
                color=ext_color, linestyle='-', linewidth=1.5, 
                zorder=15)
        ...
        for i, part_name in enumerate(FLUTE_PARTS_ORDER):
                    ...
                    if isinstance(hole_info, dict):
                        ...
                        if h_pos_rel is not None and h_diam is not None:
                            ...
                            if abs_hole_pos not in seen_hole_positions:
                                ...
                                is_cone_from_format = isinstance(h_diam, (list, tuple)) and len(h_diam) == 2
                                
                                if is_cone_from_format:
                                    # Agujero cónico definido directamente: [diam_out, diam_in]
                                    diam_out, diam_in = float(h_diam[0]), float(h_diam[1])
                                    r_outer_hole = diam_out / 2.0  # Radio en superficie externa
                                    r_inner_hole = diam_in / 2.0   # Radio en superficie interna
                                elif hole_cone_angle is not None:
                                    # Agujero cónico definido por ángulo (método antiguo)
                                    r_outer_hole = float(h_diam) / 2.0 if isinstance(h_diam, (int, float)) else 7.0 / 2.0
                                    ...
                                    diam_mm = float(h_diam) if isinstance(h_diam, (int, float)) else 7.0
                                    r_outer_hole = diam_mm / 2.0
                                ...
                                # IMPORTANTE: El radio interno del agujero NO puede ser mayor que el radio interno del tubo
                                # Esto previene que el polígono del agujero modifique visualmente el perfil interno
                                r_inner_hole = min(r_inner_hole, r_int - 0.1)  # Limitar a r_int para no modificar el perfil
                                r_inner_hole = max(r_inner_hole, 0.1)  # Mínimo razonable
                                
                                # Para cilindro, usar el mismo radio en ambas posiciones
                                if not is_cone_from_format and hole_cone_angle is None:
                                ...
                                # zorder menor que los perfiles (15) para que los perfiles se dibujen encima
            ...
            # Procesar formato 1 (Holes position/diameter) si no se usó formato 2
                ...
                # Buscar ángulos de conicidad en los datos de la parte (método antiguo)
                hole_cone_angles = part_data.get("Holes cone_angle", [])
                    ...
                    if h_pos_rel is not None and h_diam is not None:
                        ...
                        if abs_hole_pos not in seen_hole_positions:
                            ...
                            if is_cone_from_format:
                                ...
                                r_outer_hole = diam_out / 2.0  # Radio en superficie externa
                                r_inner_hole = diam_in / 2.0   # Radio en superficie interna
                            elif idx < len(hole_cone_angles) and hole_cone_angles[idx] is not None:
                                # Agujero cónico definido por ángulo (método antiguo)
                                diam_mm = float(h_diam) if isinstance(h_diam, (int, float)) else 7.0
                                r_outer_hole = diam_mm / 2.0
                            ...
                            # Para cilindro, usar el mismo radio en ambas posiciones
                            if not is_cone_from_format and (idx >= len(hole_cone_angles) or hole_cone_angles[idx] is None):
                                # Cilindro: usar el mismo radio en r_ext y r_int
                                hole_radius = min(r_outer_hole, r_inner_hole)
                                r_outer_hole = hole_radius
                                r_inner_hole = hole_radius
        ...
        ax.set_aspect('equal', adjustable='box')
        ax.grid(True, linestyle=':', alpha=0.5)
        # Posicionar leyenda fuera del área del gráfico para que no moleste
    ...
    def plot_individual_part_axial_cut_2d(self, part_name: str, ax: Optional[plt.Axes] = None,
        ...
        # Dibujar agujeros como polígonos que muestran el corte a través de la pared
        # zorder menor que los perfiles para que los perfiles se dibujen encima
        hole_positions = part_data.get("Holes position", [])
        hole_diameters = part_data.get("Holes diameter", [])
        side_holes = part_data.get("Side holes", [])
        ...
        **kwargs: Any) -> None:
        ...
        try:
            ...
            for hole_detail in holes_info:
                ...
                rad_inner_m = hole_detail.get('radius_inner_m', None)  # None para cilindros
                is_cone = hole_detail.get('is_cone', False)
                ...
                if is_open:
                    ...
                    if is_cone and rad_inner_m is not None:
                        # Agujero cónico: dibujar círculo externo e interno
                        rad_inner_plot = rad_inner_m * mmeter_conversion
                        # Círculo externo (más pequeño, diámetro externo)
                        circle_outer = Circle(
                        ...
                        # Círculo interno (más grande, diámetro interno)
                        circle_inner = Circle(
                            (x_center_plot, 0), rad_inner_plot,
                            facecolor='none',
                            edgecolor='blue',
                            linewidth=1.5,
                            linestyle='--',
                        ...
                        # Agujero cilíndrico: círculo simple
                        circle = Circle(
                            (x_center_plot, 0), rad_plot,
                            facecolor='none',
                            edgecolor='blue',
                ...
                if show_labels and hole_label:
                    ...
                    label_y = (rad_inner_plot * 2.0 if is_cone and rad_inner_m is not None else rad_plot * 2.0)
                    ax.text(x_center_plot, label_y, hole_label, 
                            ...
                            for part_name in parts_to_check:
                                ...
                                if hole_positions and hole_diameters and not side_holes:
                                    if len(hole_positions) == len(hole_diameters):
                                        for idx, (pos_rel_mm, diam_spec) in enumerate(zip(hole_positions, hole_diameters)):
                                            if pos_rel_mm is not None and diam_spec is not None:
                                                ...
                                                if is_cone:
                                                    # Agujero cónico: usar diámetro externo para el círculo principal
                                                    diam_out, diam_in = float(diam_spec[0]), float(diam_spec[1])
                                                    rad_m = (diam_out / 2.0) / 1000.0  # Radio externo
                                                    rad_inner_m = (diam_in / 2.0) / 1000.0  # Radio interno
                                                else:
                                                    # Agujero cilíndrico
                                                    diam_mm = float(diam_spec) if isinstance(diam_spec, (int, float)) else 7.0
                                                    rad_m = (diam_mm / 2.0) / 1000.0
                                                    rad_inner_m = None  # No hay radio interno para cilindro
                                                ...
                                                logger.debug(f"  Agujero en {part_name}, posición {pos_m*1000:.2f}mm: label={hole_label}, is_open={is_open}, is_cone={is_cone}")
                                                
                                                holes_details_for_plot.append({
                                                    'label': hole_label,
                                                    'position_m': pos_m,
                                                    'radius_m': rad_m,
                                                    'radius_inner_m': rad_inner_m,  # None para cilindros
                                                    'is_open': is_open,
                                                    'is_cone': is_cone
                                                })
                                ...
                                if side_holes:
                                    for hole_info in side_holes:
                                        if isinstance(hole_info, dict):
                                            ...
                                            diam_spec = hole_info.get("diameter", hole_info.get("Diameter", None))
