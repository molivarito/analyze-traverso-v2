...
# Constantes para validación de agujeros cónicos
MAX_UNDERCUT_ANGLE_DEG = 15.0  # Ángulo máximo típico de undercut en grados
...
class FluteData:
    ...
    def get_openwind_geometry_inputs(self) -> Tuple[List[List[Union[float, str]]], List[List[Any]], List[List[str]]]:
        ...
        if emb_hole_positions and emb_hole_diameters:
            ...
            # Detectar si la embocadura es cónica (lista [diam_out, diam_in]) o cilindro (número)
            emb_is_cone = isinstance(emb_diam_spec, (list, tuple)) and len(emb_diam_spec) == 2
            
            if emb_is_cone:
                # Embocadura cónica: [diam_out, diam_in]
                emb_diam_out_mm, emb_diam_in_mm = float(emb_diam_spec[0]), float(emb_diam_spec[1])
                
                if emb_diam_out_mm > 1e-9 and emb_diam_in_mm > 1e-9:
                    if emb_chim_mm < 1e-9: emb_chim_mm = DEFAULT_EMBOUCHURE_CHIMNEY_HEIGHT * 1000
                    
                    # Calcular espesor del muro (altura del cono)
                    wall_thickness_mm = self._calculate_wall_thickness_at_position(FLUTE_PARTS_ORDER[0], emb_pos_mm)
                    height_m = wall_thickness_mm / 1000.0
                    
                    # Radios en metros
                    radius_top_m = (emb_diam_out_mm / 2.0) / 1000.0  # Radio en superficie externa
                    radius_bottom_m = (emb_diam_in_mm / 2.0) / 1000.0  # Radio en superficie interna
                    ...
                    # Formato OpenWind para cono: [label, x_pos, height, radius_top, radius_bottom, "linear"]
                    side_holes_for_openwind.append([
                        embouchure_label_const, emb_pos_for_openwind_m, height_m,
                        radius_top_m, radius_bottom_m, "linear"
                    ])
                    logger.debug(f"  Conical embouchure '{embouchure_label_const}' added: pos_rel_corcho={emb_pos_for_openwind_m:.4f}m, "
                               f"diam_out={emb_diam_out_mm:.2f}mm, diam_in={emb_diam_in_mm:.2f}mm, wall_thickness={wall_thickness_mm:.2f}mm")
            else:
                # Embocadura cilíndrica: número simple
                emb_diam_mm = float(emb_diam_spec) if isinstance(emb_diam_spec, (int, float)) else 7.0
                ...
                # Formato OpenWind para cilindro: [label, x_pos, height, radius, radius_out]
                side_holes_for_openwind.append([
                    embouchure_label_const, emb_pos_for_openwind_m, emb_chim_mm / 1000.0,
                    (emb_diam_mm / 2.0) / 1000.0, (emb_diam_o_mm / 2.0) / 1000.0
                ])
                logger.debug(f"  Cylindrical embouchure '{embouchure_label_const}' added: pos_rel_corcho={emb_pos_for_openwind_m:.4f}m, "
                           f"diam={emb_diam_mm:.2f}mm")
        else:
        ...
        for part_name in FLUTE_PARTS_ORDER:
            ...
            for i, hole_pos_rel_mm in enumerate(holes_pos_rel_list):
                hole_diam_spec = holes_diam_rel_list[i] if i < len(holes_diam_rel_list) else 7.0
                ...
                # Detectar si el agujero es cilindro (número) o cono (lista [diam_out, diam_in])
                is_cone = isinstance(hole_diam_spec, (list, tuple)) and len(hole_diam_spec) == 2
                
                if is_cone:
                    # Agujero cónico: [diam_out, diam_in]
                    diam_out_mm, diam_in_mm = float(hole_diam_spec[0]), float(hole_diam_spec[1])
                    
                    if diam_out_mm > 1e-9 and diam_in_mm > 1e-9:
                        if chimney_mm < 1e-9: chimney_mm = DEFAULT_CHIMNEY_HEIGHT * 1000
                        
                        # Calcular espesor del muro (altura del cono)
                        wall_thickness_mm = self._calculate_wall_thickness_at_position(part_name, hole_pos_rel_mm)
                        height_m = wall_thickness_mm / 1000.0
                        
                        # Radios en metros
                        radius_top_m = (diam_out_mm / 2.0) / 1000.0  # Radio en superficie externa
                        radius_bottom_m = (diam_in_mm / 2.0) / 1000.0  # Radio en superficie interna
                        
                        # Formato OpenWind para cono: [label, x_pos, height, radius_top, radius_bottom, "linear"]
                        side_holes_for_openwind.append([
                            hole_label, hole_pos_for_openwind_m, height_m,
                            radius_top_m, radius_bottom_m, "linear"
                        ])
                        logger.debug(f"  Conical tone hole '{hole_label}' added: part='{part_name}', pos_rel_part={hole_pos_rel_mm}mm, "
                                   f"diam_out={diam_out_mm:.2f}mm, diam_in={diam_in_mm:.2f}mm, wall_thickness={wall_thickness_mm:.2f}mm, "
                                   f"pos_rel_corcho={hole_pos_for_openwind_m:.4f}m")
                else:
                    # Agujero cilíndrico: número simple
                    diam_mm = float(hole_diam_spec) if isinstance(hole_diam_spec, (int, float)) else 7.0
                    ...
                    logger.debug(f"  Cylindrical tone hole '{hole_label}' added: part='{part_name}', pos_rel_part={hole_pos_rel_mm}mm, "
                               f"pos_abs_phys={hole_abs_physical_pos_mm:.2f}mm, pos_rel_corcho={hole_pos_for_openwind_m:.4f}m, "
                               f"diam={diam_mm:.2f}mm")
    ...
    def _calculate_wall_thickness_at_position(self, part_name: str, hole_pos_rel_mm: float) -> float:
        """
        Calcula el espesor del muro (wall thickness) en la posición de un agujero.
        El espesor se calcula como: (diam_ext - diam_int) / 2.0
        
        Args:
            part_name: Nombre de la parte (ej: 'headjoint', 'left', 'right', 'foot')
            hole_pos_rel_mm: Posición relativa del agujero en mm (relativa al inicio físico de la parte)
        
        Returns:
            Espesor del muro en mm. Si no hay datos externos, retorna un valor por defecto estimado (2.0mm).
        """
        part_data = self.data.get(part_name, {})
        if not part_data:
            logger.warning(f"No hay datos para parte '{part_name}' al calcular espesor del muro en {self.flute_model}.")
            return 2.0  # Valor por defecto estimado
        
        # Obtener diámetro interno desde measurements
        measurements = part_data.get("measurements", [])
        if not measurements:
            logger.warning(f"No hay measurements para parte '{part_name}' al calcular espesor del muro en {self.flute_model}.")
            return 2.0
        
        diam_int = self._get_diameter_from_measurements_at_pos(measurements, hole_pos_rel_mm)
        if diam_int <= 0:
            logger.warning(f"Diámetro interno inválido ({diam_int:.2f}mm) en posición {hole_pos_rel_mm:.2f}mm para parte '{part_name}' en {self.flute_model}.")
            return 2.0
        
        # Intentar obtener diámetro externo desde external_geometry si está disponible
        diam_ext = None
        if hasattr(self, 'external_geometry') and self.external_geometry:
            external_geometry = getattr(self, 'external_geometry', {})
            ...
            if external_measurements:
                try:
                    ...
                    if len(positions) > 0 and len(diameters) > 0:
                        if hole_pos_rel_mm <= positions[0]:
                            diam_ext = diameters[0]
                        elif hole_pos_rel_mm >= positions[-1]:
                            diam_ext = diameters[-1]
                        else:
                            diam_ext = float(np.interp(hole_pos_rel_mm, positions, diameters))
                except Exception as e:
                    logger.warning(f"Error interpolando diámetro externo para parte '{part_name}' en posición {hole_pos_rel_mm:.2f}mm en {self.flute_model}: {e}")
        
        # Si no hay datos externos, usar estimación basada en diámetro interno
        if diam_ext is None or diam_ext <= 0:
            # Estimación: asumir espesor de 2mm por lado (4mm total de diámetro)
            diam_ext = diam_int + 4.0
            logger.debug(f"Usando diámetro externo estimado ({diam_ext:.2f}mm) para parte '{part_name}' en posición {hole_pos_rel_mm:.2f}mm en {self.flute_model}.")
        
        wall_thickness = (diam_ext - diam_int) / 2.0
        if wall_thickness <= 0:
            logger.warning(f"Espesor del muro calculado es negativo o cero ({wall_thickness:.2f}mm) para parte '{part_name}' en posición {hole_pos_rel_mm:.2f}mm en {self.flute_model}. Usando 2.0mm por defecto.")
            return 2.0
        ...
        for part_name in FLUTE_PARTS_ORDER:
            ...
            if not isinstance(holes_pos, list) or not isinstance(holes_diam, list):
                    ...
                    if prop_list is not None:
                        ...
                        for i_h in range(num_holes):
                            if corrected_list[i_h] is None or (isinstance(corrected_list[i_h], (int,float)) and corrected_list[i_h] < 0):
                                ...
                                elif prop_name == "Holes diameter_out":
                                    # Detectar si el agujero es cónico (lista) o cilindro (número)
                                    hole_diam_spec = holes_diam[i_h]
                                    if isinstance(hole_diam_spec, (list, tuple)) and len(hole_diam_spec) == 2:
                                        # Cono: usar diámetro externo
                                        diam_val = float(hole_diam_spec[0])
                                    elif isinstance(hole_diam_spec, (int, float)):
                                        # Cilindro: usar el número directamente
                                        diam_val = float(hole_diam_spec)
                                    else:
                                        diam_val = 0.0
                                    
                                    if diam_val > 0:
                                        corrected_list[i_h] = diam_val * default_val_factor
                                else: 
                ...
                for i, h_pos_val in enumerate(holes_pos):
                    ...
                    # Validar formato de diámetro: número (cilindro) o lista [diam_out, diam_in] (cono)
                    if current_hole_diam_spec is None:
                        self.validation_errors.append({'part': part_name, 'message': f"Parte '{part_name}', agujero {i+1}: 'diameter' es None o no está definido."})
                    elif isinstance(current_hole_diam_spec, (list, tuple)):
                        # Formato de cono: [diam_out, diam_in]
                        if len(current_hole_diam_spec) != 2:
                            self.validation_errors.append({'part': part_name, 'message': f"Parte '{part_name}', agujero {i+1}: Lista de diámetro debe tener exactamente 2 elementos [diam_out, diam_in], pero tiene {len(current_hole_diam_spec)}."})
                        elif not all(isinstance(d, (int, float)) for d in current_hole_diam_spec):
                            self.validation_errors.append({'part': part_name, 'message': f"Parte '{part_name}', agujero {i+1}: Elementos de lista de diámetro deben ser números."})
                        else:
                            diam_out, diam_in = float(current_hole_diam_spec[0]), float(current_hole_diam_spec[1])
                            if diam_out <= 0 or diam_in <= 0:
                                self.validation_errors.append({'part': part_name, 'message': f"Parte '{part_name}', agujero {i+1}: Diámetros de cono deben ser positivos (diam_out={diam_out:.2f}, diam_in={diam_in:.2f})."})
                            elif diam_in < diam_out:
                                # Overcut (menos común): diam_in < diam_out
                                self.validation_warnings.append({'part': part_name, 'message': f"Parte '{part_name}', agujero {i+1}: Overcut detectado (diam_in={diam_in:.2f} < diam_out={diam_out:.2f}). Esto es menos común que undercut."})
                            # Mantener el formato de lista para conos
                            part_data["Holes diameter"][i] = [diam_out, diam_in]
                    elif isinstance(current_hole_diam_spec, (int, float)):
                        # Formato de cilindro: número simple
                        effective_hole_diam = float(current_hole_diam_spec)
                        if effective_hole_diam <= 0:
                            self.validation_errors.append({'part': part_name, 'message': f"Parte '{part_name}', agujero {i+1}: Diámetro de cilindro debe ser positivo ({effective_hole_diam:.2f})."})
                        # Mantener como número para cilindros
                    else:
                        self.validation_errors.append({'part': part_name, 'message': f"Parte '{part_name}', agujero {i+1}: 'diameter' debe ser un número (cilindro) o lista [diam_out, diam_in] (cono), pero es {type(current_hole_diam_spec).__name__}."})

            if is_headjoint:
                # Si ya existe la posición calculada (por ejemplo, desde BD), no recalcular
                if '_calculated_stopper_absolute_position_mm' in part_data:
                    calculated_stopper_abs_pos_mm = part_data['_calculated_stopper_absolute_position_mm']
                    if calculated_stopper_abs_pos_mm is not None:
                        logger.debug(f"{self.flute_model}: Usando posición del corcho ya calculada desde BD: {calculated_stopper_abs_pos_mm:.2f}mm.")
                        # Validar que la posición sea válida
                        if calculated_stopper_abs_pos_mm < 0:
                            self.validation_errors.append({'part': part_name, 'message': f"Parte '{part_name}': Posición absoluta del corcho ({calculated_stopper_abs_pos_mm:.2f}mm) es negativa. Revisar datos."})
                        elif total_length > 0 and calculated_stopper_abs_pos_mm >= total_length:
                             self.validation_errors.append({'part': part_name, 'message': f"Parte '{part_name}': Posición absoluta del corcho ({calculated_stopper_abs_pos_mm:.2f}mm) excede o iguala la longitud total del headjoint ({total_length:.2f}mm)."})
                    return  # Ya está calculado, no hacer nada más
                
                # Si no existe, calcular como antes
        ...
        try:
            ...
            if emb_hole_diameters:
                ...
                if emb_diam_mm > 0:
                    ...
                    logger.warning(f"No se encontró diámetro de embocadura válido para {self.flute_model}, usando Rw por defecto {Rw}m.")
            else:
                logger.warning(f"No se encontró diámetro de embocadura válido para {self.flute_model}, usando Rw por defecto {Rw}m.")

            freq_range = np.arange(100, 3000, 2.0)  # Rango completo para análisis detallado de impedancia
            player = Player("FLUTE")
            # Para flauta traversa, Player("FLUTE") ya configura los parámetros de radiación apropiados
