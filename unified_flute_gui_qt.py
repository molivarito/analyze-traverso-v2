...
try:
    ...
    if "OCP" in error_msg or "No module named 'OCP'" in error_msg:
        print("Warning: CadQuery no disponible - falta la dependencia OCP (OpenCASCADE Python).")
        print("  CadQuery está instalado pero requiere OCP para funcionar.")
        print(f"  Python actual: {python_path}")
        
        # Verificar si hay un entorno conda OpenWind disponible
        import os
        conda_env_path = "/Users/pdelac/miniforge3/envs/OpenWind/bin/python"
        if os.path.exists(conda_env_path):
            # Verificar si PyQt5 está disponible en el entorno conda
            try:
                import subprocess
                result = subprocess.run(
                    [conda_env_path, "-c", "import PyQt5; print('OK')"],
                    capture_output=True,
                    text=True,
                    timeout=5
                )
                has_pyqt5 = "OK" in result.stdout
            except Exception:
                has_pyqt5 = False
            
            if has_pyqt5:
                print("  NOTA: Se detectó entorno conda OpenWind con OCP y PyQt5 instalados.")
                print("  Solución: Usa el Python del entorno conda directamente:")
                print(f"    {conda_env_path} unified_flute_gui_qt.py")
                print("  O activa correctamente el entorno:")
                print("    conda activate OpenWind")
                print("    python unified_flute_gui_qt.py")
            else:
                print("  NOTA: Se detectó entorno conda OpenWind con OCP instalado, pero falta PyQt5.")
                print("  Solución: Instala PyQt5 en el entorno conda OpenWind:")
                print("    conda activate OpenWind")
            ...
            print("  Soluciones:")
            print("  1. Instalar OCP: conda install -c conda-forge ocp")
            print("  2. O usar Python 3.11/3.12 en lugar de 3.13")
    else:
        print(f"Warning: CadQuery no disponible. Error: {error_msg}")
        print("  Instala con: pip install cadquery")
        print("  O mejor con conda: conda install -c conda-forge cadquery")
...
# Imports de módulos de refactorización
...
class FluteAssembler3D:
    ...
    def assemble(self):
        ...
        for i in range(self.internal_data.get("Number of holes", 0)):
            ...
            diam_spec = self.internal_data["Holes diameter"][i]
            ...
            if is_cone:
                # Agujero cónico: usar diámetros definidos directamente
                diam_out, diam_in = float(diam_spec[0]), float(diam_spec[1])
                r_outer_hole = diam_out / 2.0
                r_inner_hole = diam_in / 2.0
            else:
                # Agujero cilíndrico: calcular cono basado en ángulo
                d_outer_hole = float(diam_spec) if isinstance(diam_spec, (int, float)) else 7.0
...
# ==================== Diálogo de Reporte Estadístico ====================

class DatabaseStatisticsDialog(QDialog):
    """Diálogo para configurar y generar reporte estadístico de la base de datos."""
    
    def __init__(self, db_manager, parent=None):
        super().__init__(parent)
        self.db_manager = db_manager
        self.setWindowTitle("Reporte Estadístico de Base de Datos")
        self.setMinimumSize(700, 600)
        
        layout = QVBoxLayout(self)
        
        # Título
        title_label = QLabel("Generar Reporte Estadístico Completo")
        title_font = QFont()
        ...
        # Sección 1: Selección de flautas
        flutes_group = QGroupBox("Filtro de Flautas")
        flutes_layout = QVBoxLayout(flutes_group)
        
        # Obtener lista de flautas
        try:
            all_flutes = self.db_manager.get_all_flutes_for_statistics()
        except Exception as e:
            logger.error(f"Error obteniendo flautas: {e}")
            all_flutes = []
        ...
        flutes_layout.addLayout(select_all_layout)
        
        # Lista de flautas con checkboxes
        self.flute_list = QListWidget()
        self.flute_list.setSelectionMode(QListWidget.MultiSelection)
        
        for flute in all_flutes:
            item = QListWidgetItem(flute['flute_model'])
            item.setFlags(item.flags() | Qt.ItemIsUserCheckable)
            item.setCheckState(Qt.Checked)
            self.flute_list.addItem(item)
        
        flutes_layout.addWidget(self.flute_list)
        scroll_layout.addWidget(flutes_group)
        
        # Sección 2: Métricas a incluir
        metrics_group = QGroupBox("Métricas a Incluir")
        metrics_layout = QVBoxLayout(metrics_group)
        
        self.metrics_checkboxes = {}
        metrics_options = [
            ('include_slopes', 'Pendientes por Parte', True),
            ('include_holes', 'Tamaños de Agujeros', True),
            ('include_physical_lengths', 'Largos Físicos por Parte', True),
            ('include_embouchure', 'Diámetro de Embocadura', True),
            ('include_acoustic_length', 'Largo Acústico Total', True),
            ('include_correlations', 'Análisis de Correlaciones', False),
        ]
        
        for key, label, default in metrics_options:
            checkbox = QCheckBox(label)
            checkbox.setChecked(default)
            self.metrics_checkboxes[key] = checkbox
            metrics_layout.addWidget(checkbox)
        
        scroll_layout.addWidget(metrics_group)
        
        # Sección 3: Opciones de formato
        format_group = QGroupBox("Opciones de Formato")
        format_layout = QFormLayout(format_group)
        
        # Incluir tablas estadísticas
        self.include_tables_checkbox = QCheckBox("Incluir tablas estadísticas")
        self.include_tables_checkbox.setChecked(True)
        format_layout.addRow("Tablas:", self.include_tables_checkbox)
        
        # Incluir resumen ejecutivo
        self.include_summary_checkbox = QCheckBox("Incluir resumen ejecutivo")
        self.include_summary_checkbox.setChecked(True)
        format_layout.addRow("Resumen:", self.include_summary_checkbox)
    ...
    def _toggle_select_all(self, state):
        """Selecciona o deselecciona todas las flautas."""
        check_state = Qt.Checked if state == Qt.Checked else Qt.Unchecked
        for i in range(self.flute_list.count()):
            item = self.flute_list.item(i)
            item.setCheckState(check_state)
    
    def _generate_report(self):
        """Genera el reporte estadístico con las opciones seleccionadas."""
        try:
            # Obtener flautas seleccionadas
            selected_flutes = []
            for i in range(self.flute_list.count()):
                item = self.flute_list.item(i)
                if item.checkState() == Qt.Checked:
                    selected_flutes.append(item.text())
            
            if not selected_flutes:
                QMessageBox.warning(self, "Advertencia", "Debe seleccionar al menos una flauta.")
                return
            
            # Preparar configuración
            config = {
                'include_slopes': self.metrics_checkboxes['include_slopes'].isChecked(),
                'include_holes': self.metrics_checkboxes['include_holes'].isChecked(),
                'include_physical_lengths': self.metrics_checkboxes['include_physical_lengths'].isChecked(),
                'include_embouchure': self.metrics_checkboxes['include_embouchure'].isChecked(),
                'include_acoustic_length': self.metrics_checkboxes['include_acoustic_length'].isChecked(),
                'include_correlations': self.metrics_checkboxes['include_correlations'].isChecked(),
            ...
            default_filename = f"reporte_estadistico_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
            filename, _ = QFileDialog.getSaveFileName(
                self,
            ...
            progress = QProgressDialog("Generando reporte estadístico...", "Cancelar", 0, 100, self)
            ...
            from database_statistics import DatabaseStatisticsExtractor, StatisticsReportGenerator
            
            # Extraer métricas
            progress.setLabelText("Extrayendo métricas de la base de datos...")
            progress.setValue(30)
            QApplication.processEvents()
            
            extractor = DatabaseStatisticsExtractor(self.db_manager)
            metrics_data = extractor.extract_all_flutes_metrics(selected_flutes)
            
            if not metrics_data:
                QMessageBox.warning(self, "Advertencia", "No se pudieron extraer métricas de las flautas seleccionadas.")
                progress.close()
                return
            
            # Generar reporte
            progress.setLabelText("Generando PDF...")
            progress.setValue(60)
            QApplication.processEvents()
            
            generator = StatisticsReportGenerator(metrics_data)
            generator.generate_report(Path(filename), config)
            ...
            msg.setText(f"Reporte estadístico generado exitosamente:\n\n{filename}\n\nFlautas analizadas: {len(metrics_data)}")
            msg.setStandardButtons(QMessageBox.Ok | QMessageBox.Open)
            
            result = msg.exec_()
            
            # Abrir el PDF si el usuario lo solicita
            ...
            logger.error(f"Error generando reporte estadístico: {e}", exc_info=True)
...
class UnifiedFluteGUI_Qt(QMainWindow):
    ...
    def __init__(self):
        try:
            ...
            # Referencia al editor de geometría (para que no se destruya cuando es no-modal)
            self.geometry_editor: Optional[FluteGeometryEditor] = None
            ...
            # Inicializar PlotUpdater después de crear la UI
            self.plot_updater = PlotUpdater(self)
            logger.info("PlotUpdater inicializado")
    ...
    def _create_menu_bar(self):
        ...
        stats_report_action = report_menu.addAction("Reporte Estadístico Completo...")
        stats_report_action.triggered.connect(self._open_statistics_report_dialog)
        ...
        sensitivity_action = analysis_menu.addAction("Análisis de Sensibilidad...")
        sensitivity_action.triggered.connect(self._open_sensitivity_analysis_dialog)
    ...
    def _open_geometry_editor(self):
        ...
        try:
            # Si ya hay un editor abierto, traerlo al frente
            if self.geometry_editor is not None and self.geometry_editor.isVisible():
                self.geometry_editor.raise_()
                self.geometry_editor.activateWindow()
                return
            
            # Crear y abrir editor (no-modal)
            editor = FluteGeometryEditor(selected_flute, self)
            
            # Guardar referencia para que no se destruya
            self.geometry_editor = editor
            ...
            # NUEVA: Conectar señal para carga en análisis
            editor.flute_loaded_for_analysis.connect(self._on_editor_flute_loaded_for_analysis)
            
            # Conectar señal cuando se cierre el editor para limpiar la referencia
            editor.finished.connect(lambda: setattr(self, 'geometry_editor', None))
            
            # Mostrar editor (no-modal, no bloquea)
            editor.show()
    ...
    def _open_sensitivity_analysis_dialog(self):
        """Abre el diálogo de análisis de sensibilidad."""
        if not self.flute_ops_list:
            QMessageBox.warning(
                self, "Sin Flautas",
                "Debe cargar al menos una flauta base para el análisis de sensibilidad."
            )
            return
        
        try:
            # Crear y abrir diálogo
            dialog = SensitivityAnalysisDialog(self.flute_ops_list, self)
            
            # Conectar señal para cargar variantes cuando estén listas
            dialog.variants_ready.connect(self._load_sensitivity_variants)
            
            # Abrir diálogo (no bloquea - las variantes se cargan cuando se generan)
            dialog.exec_()
            
            # Las variantes ya se cargaron cuando se generaron (vía señal variants_ready)
        
        except Exception as e:
            logger.error(f"Error abriendo diálogo de análisis de sensibilidad: {e}", exc_info=True)
            QMessageBox.critical(
                self, "Error",
                f"Error abriendo diálogo de análisis de sensibilidad:\n{e}"
            )
    
    def _load_sensitivity_variants(self, variants: List[FluteDataDB]):
        """Carga variantes de sensibilidad en la GUI."""
        if not variants:
            return
        ...
        try:
            ...
            # Añadir variantes a las listas existentes
            for variant in variants:
                self.flute_data_list.append(variant)
                self.flute_ops_list.append(FluteOperations(variant))
            
            # Actualizar visualizaciones
            self._ensure_finger_frequencies()
            
            # Recrear analizador con todas las flautas
            ...
            QMessageBox.information(
                self, "Variantes Cargadas",
                f"Se cargaron {len(variants)} variantes exitosamente.\n\n"
                f"Ahora puedes visualizar y comparar las variantes usando las pestañas de análisis."
            )
        
        except Exception as e:
            QApplication.restoreOverrideCursor()
            logger.error(f"Error cargando variantes de sensibilidad: {e}", exc_info=True)
            QMessageBox.critical(
                self, "Error",
                f"Error cargando variantes:\n{e}"
            )
    
    def _on_editor_flute_loaded_for_analysis(self, temp_flute: FluteDataDB):
        """
        Maneja la carga de una flauta editada para análisis completo.
        Similar a _load_sensitivity_variants pero para una sola flauta del editor.
        """
        try:
            QApplication.setOverrideCursor(Qt.WaitCursor)
            ...
            logger.info(f"Flauta editada '{temp_flute.flute_model}' cargada para análisis")
        
        except Exception as e:
            QApplication.restoreOverrideCursor()
            logger.error(f"Error cargando flauta editada: {e}", exc_info=True)
            QMessageBox.critical(
                self, "Error",
                f"Error cargando flauta editada:\n{e}"
            )
        ...
        try:
            ...
            for part_name in FLUTE_PARTS_ORDER:
                ...
                if hole_positions and hole_diameters:
                    for pos_rel_mm, diam_spec in zip(hole_positions, hole_diameters):
                        abs_pos_mm = part_start_abs + pos_rel_mm
                        
                        # Detectar si es cónico (lista) o cilindro (número)
                        if isinstance(diam_spec, (list, tuple)) and len(diam_spec) == 2:
                            # Cono: usar diámetro externo para el tamaño del marcador
                            diam_mm = float(diam_spec[0])
                        else:
                            # Cilindro: usar el número directamente
                            ...
                            for h_pos, h_diam in zip(hole_positions, hole_diameters):
                                if h_pos is not None and h_diam is not None:
                                    ...
                                    if isinstance(h_diam, (list, tuple)) and len(h_diam) == 2:
                                        # Cono: usar diámetro externo para el tamaño del marcador
                                        diam_for_marker = float(h_diam[0])
                                    else:
                                        # Cilindro: usar el número directamente
                                        diam_for_marker = float(h_diam) if isinstance(h_diam, (int, float)) else 0.0
                                    marker_size = max(diam_for_marker * 2.0, 4)
                                    ax_part.plot(
    ...
    def _update_analysis_plots(self):
        ...
        if not self.analyzer or not self.plot_updater:
            return
        
        # 1. Resumen (Dashboard)
        self.plot_updater.update_summary_dashboard()
    ...
    def _update_summary_dashboard(self):
        ...
        try:
            ...
            all_flute_names = set()
            if self.flute_data_list:
                all_flute_names = {fd.flute_model for fd in self.flute_data_list}
            ...
            q_factor_data = {}
            if self.analyzer:
                for analysis_dict, flute_name in self.analyzer.acoustic_analysis_list:
                    q_factors = []
                    for note in self.analyzer.ordered_notes:
    ...
    def _open_statistics_report_dialog(self):
        """Abre el diálogo para configurar y generar el reporte estadístico."""
        if not self._check_db_manager():
            return
        
        try:
            dialog = DatabaseStatisticsDialog(self.db_manager, self)
            dialog.exec_()
        except Exception as e:
            logger.error(f"Error abriendo diálogo de reporte estadístico: {e}", exc_info=True)
            QMessageBox.critical(
                self,
                "Error",
                f"Error abriendo diálogo de reporte estadístico:\n{e}"
            )
